...
class MNAM_prob_soft(pl.LightningModule):
    def __init__(
        self,
        num_inputs: int,
        num_units: list,
        ...
        super(MNAM_prob_soft, self).__init__()
    ...
    def compute_loss(self, x, y):
        ...
        dropout_list_mu = []
        dropout_list_sigma = []
        # track gaussian likelihood for soft thresholding
        gau_lik_list = []
        for i in range(self.num_gaus):
            z_mu_ = torch.cat(
                torch.split(z_mu[:, i].view(-1, 1), int(len(z_mu) / self.num_inputs)),
            ...
            gau_lik_list.append(torch.exp(normal_prob(mu, y, sigma)).view(-1, 1))
        ...
        # gaussian likelihood to calculate pi
        pi_prob_hat = F.softmax(pi_hat)
        gau_like = torch.cat(gau_lik_list, dim=1)

        pxz = pi_prob_hat * gau_like
        px = torch.sum(pxz, axis=1)
        pi = torch.div(pxz, px.view(-1, 1))

        # add loss by weights which is pi and compute cross entropy loss
        loss = torch.sum(loss_eval * pi, axis=1)
        loss_cent = self.eval_loss_cent(pi_hat, pi)
        ...
        if self.output_penalty > 0:
            loss += (
                torch.sum(dropout_mu**2)
                / (dropout_mu.size()[0] * dropout_mu.size()[1])
                * self.output_penalty
            )
            loss += (
                torch.sum(feature_output_sigma**2)
                / (dropout_sigma.size()[0] * dropout_sigma.size()[1])
                * self.output_penalty
            )
    ...
    def compute_test_error_reg(self, x, y):
        ...
        return torch.mean(loss), stats.wasserstein_distance(
            y.detach().cpu().numpy(), np.concatenate(ypred)
        )
...
# helper function
def normal_prob(mu, y, var):
    scale = var**0.5
    log_scale = math.log(scale) if isinstance(scale, Real) else scale.log()
    return -((y - mu) ** 2) / (2 * var) - log_scale - math.log(math.sqrt(2 * math.pi))
